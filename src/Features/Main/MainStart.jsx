...
const ALL_MODULES = [
  { name: "System", items: ["User Management", "Roles & Permissions", "System Settings", "Audit Logs", "Integrations"] },
              ...
              {filteredModules.map((m) => {
                ...
                return (
                  <div key={m.name}>
                    ...
                    {m.items.filter(i => i.toLowerCase().includes(searchQuery.toLowerCase())).map(item => (
                      <button
                        key={item}
                        onClick={() => { setActivePage({ module: m.name, item }); setSearchQuery(""); }}
                  ...
                  <div key={modName}>
                    ...
                    {isOpen && (
                      <div className="submenu-enter ml-5 mr-2 mb-1 border-l-2 border-blue-100 pl-3">
                        {mod.items.map((item) => {
                          const isItemActive = activePage.module === modName && activePage.item === item;
                          return (
                            <button
                              key={item}
                              onClick={() => setActivePage({ module: modName, item })}
